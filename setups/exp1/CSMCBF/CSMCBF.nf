...
echo true
  ...
  input:
    ...
    each mainRand from  50
    each rand from  11
    each particle from 1
    each taxa from (10, 40, 100)
    each iterScalings from (1, 3, 10, 30)
    echo true
...
java   -cp 'code/lib/*' -Xmx16G   phyloPMCMC.PGSExperiments -useDataGenerator true    -nThousandIters 1 \
     -nTax $taxa  \
     -treeRate 10\
     -mainRand $mainRand  \
     -len  5000 \
...
process aggregate {
  ...
  ./code/bin/csv-aggregate \
    ...
    --argumentsKeys    gen.rand  PGSExperiments.iterScalings PGSExperiments.methods PGSExperiments.mainRand PGSExperiments.nParticlesEachStep gen.nTaxa\
