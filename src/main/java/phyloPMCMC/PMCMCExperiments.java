...
public class PMCMCExperiments implements Runnable {
	...
	@Option
	public ArrayList<InferenceMethod> methods = list(Arrays.asList(InferenceMethod.PMMHNC, InferenceMethod.SMC));
	@Option
...
//		},
		 ...
		 * ParticleFilter<PartialCoalescentState>(); // ParticleFilter<LazyPCS> pc = new
		 * ParticleFilter<LazyPCS>(); pc.N = (int) (iterScale * instance.nThousandIters
		 * * 1000); pc.nThreads = 8; pc.resampleLastRound = true; pc.rand =
		 * instance.mainRand; pc.verbose = instance.verbose; //
		 ...
		 * // KernelType kernelType = KernelType.PRIOR_PRIOR;
		 * 
		 * CTMC ctmc = CTMC.SimpleCTMC.dnaCTMC(dataset.nSites()); PartialCoalescentState
		 * init = PartialCoalescentState.initFastState(dataset, ctmc, false);
		 * ParticleKernel<PartialCoalescentState> ppk = new NCPriorPriorKernel(init);
		 * TreeDistancesProcessor tdp = new TreeDistancesProcessor();
		 * 
		 * pc.sample(ppk,tdp); System.out.println("initial loglikelihood: "+init.
		 * logLikelihood()+" Marginal loglikelihood: "
		 * +(pc.estimateNormalizer()+init.logLikelihood())); return tdp; } }, LAZYNCPF {
		 * 
		 * @Override public TreeDistancesProcessor doIt(PMCMCExperiments instance,
		 * double iterScale, UnrootedTree goldut, String treeName) {
		 * ParticleFilter<LazyPCS> pc = new ParticleFilter<LazyPCS>(); pc.N = (int)
		 * (iterScale * instance.nThousandIters * 1000); pc.nThreads = 1;
		 * pc.resampleLastRound = true; pc.rand = instance.mainRand; Dataset dataset =
		 * DatasetUtils.fromAlignment(instance.data, instance.sequenceType); CTMC ctmc =
...
//		},
		 ...
		 * instance.sequenceType); CTMC ctmc =
		 * CTMC.SimpleCTMC.dnaCTMC(dataset.nSites()); PartialCoalescentState init =
		 * PartialCoalescentState.initFastState(dataset, ctmc); LazyParticleKernel pk2 =
		 * new PriorPriorKernel(init); LazyParticleFilter<PartialCoalescentState> lpf =
		 * new LazyParticleFilter<PartialCoalescentState>(pk2, options);
		 * TreeDistancesProcessor tdp = new TreeDistancesProcessor(); double
		 * initTrans2tranv=
		 * Math.exp(Math.log(2.0)+instance.mainRand.nextGaussian()*0.1); PMMHSMC2 pmmh =
		 * new PMMHSMC2(dataset, options, tdp, initTrans2tranv);
		 * pmmh.a=instance.parameter_a; // final double requested = (iterScale *
		 * instance.nThousandIters * 1000); // pc.N = 1+ (int) Math.pow(requested,
		 * instance.pmcmcSMCExpMix); // final int nMCMC = 1 + (int) Math.pow(requested,
		 * 1.0 - instance.pmcmcSMCExpMix); final int nMCMC=(int) iterScale;
		 * System.out.println("# particles: "+ options.nParticles +"; nMCMC:"+ nMCMC);
		 * int i=0; // for (int i = 0; i < nMCMC; i++) while(!pmmh.stop && i<nMCMC) {
		 * i++; pmmh.next(instance.mainRand); } return tdp; } },
		 */
